...
Handles interactive commands: /health, /today
"""
import asyncio
import logging
...
class TelegramCommandHandler:
    ...
    async def cmd_health(self):
        ...
        try:
            health = self.db.check_heartbeat_status(timeout_minutes=3, critical_minutes=10)
...
"""
    ...
    async def cmd_today(self):
        ...
        try:
            ...
            completed_signals = [s for s in signals if s.trade_iq and s.status in ["WIN", "LOSS", "TIMEOUT"]]
            avg_iq = sum(s.trade_iq for s in completed_signals) / len(completed_signals) if completed_signals else 0
...
"""
            ...
            if completed_signals:
                ...
                for i, signal in enumerate(completed_signals[-3:], 1):  # Last 3 trades
                    result_emoji = "✅" if signal.status == "WIN" else "❌" if signal.status == "LOSS" else "⏱️"
