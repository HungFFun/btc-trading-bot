...
OPTIMIZED v5.1.1 - Chỉ gửi 2 thông báo quan trọng:
1. 🔔 NEW SIGNAL - Khi có tín hiệu trading (QUAN TRỌNG NHẤT)
2. ⚠️ ERROR - Khi có lỗi nghiêm trọng

KHÔNG GỬI:
- Regime change (quá nhiều noise)
- Daily start/limits (Bot 2 handles)
- Learning insights (chỉ log)
"""
...
class TelegramBot:
    ...
    async def send_message(self, text: str, parse_mode: str = "HTML") -> bool:
        ...
        if not self.token or not self.chat_id:
            logger.error(
                f"Telegram not configured: token={bool(self.token)}, chat_id={bool(self.chat_id)}"
            )
            return False
        ...
        try:
            ...
            async with session.post(url, json=data) as response:
                ...
                if response.status == 200:
                    ...
                    logger.error(
                        f"❌ Telegram error (status={response.status}): {response_text}"
                    )
                    return False
    ...
    # =====================================================
    # THÔNG BÁO CHÍNH - Chỉ giữ 2 loại quan trọng
    # 1. NEW SIGNAL - Khi có tín hiệu trading
    # 2. ERROR - Khi có lỗi nghiêm trọng
    # =====================================================
...
"""
    ...
    async def send_regime_change(
        self, old_regime: str, new_regime: str, confidence: float, details: Dict = None
    ) -> bool:
        """
...
"""
    ...
    async def send_daily_start(self) -> bool:
        ...
        logger.debug(
            "send_daily_start() deprecated - Bot 2 handles new day notifications"
        )
        return True
    ...
    async def send_daily_limit_reached(self, limit_type: str, pnl: float) -> bool:
        """DEPRECATED - Bot 2 handles this"""
        logger.debug(
            f"send_daily_limit_reached() deprecated - Bot 2 handles {limit_type}"
        )
        return True
    ...
    async def send_learning_insight(self, lesson) -> bool:
        ...
        logger.info(
            f"Learning insight: {lesson.observation[:100] if lesson else 'N/A'}"
        )
        return True
